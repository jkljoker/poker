...
import joker3.domain.Poker;
import joker3.domain.User;
import joker3.util.CodeUtil;
...
public class LoginJFrame extends JFrame implements MouseListener {
    ...
    static {
        allUsers.add(new User("xdx","123456"));
        allUsers.add(new User("joker","123456"));
    }
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        if (source == login) {
            ...
            String passwordTnput = password.getText();
            String codeInput = code.getText();
            if (codeInput.length() == 0) {
                showJDialog("验证码不能为空");
                return;
            }
            //直接定义一个User对象，然后调用allusers中的contains方法判断是否有该用户
            User u = new User(userNameInput, passwordTnput);
            if (!select(u)){//如果不存在
                showJDialog("用户名或密码有误");
            } else if (!codeInput.equals(rightCode.getText())) {//验证码输入有误
                showJDialog("验证码输入有误");
            } else {//用户名密码验证码都正确，启动游戏
                this.setVisible(false);
                new GameJFrame();
            }
        } else if (source == register) {//如果是注册按钮,此处功能尚未学习，以system语句替换
    ...
    public boolean select(User u) {
        for (User allUser : allUsers) {
            if (u.getUserName().equals(allUser.getUserName()) && u.getPassword().equals(allUser.getPassword())) {
                return true;
            }
        }
        return false;
